...
```python
from image_gen_mcp.apps.cell_cover.utils.image_uploader import upload_to_imgbb
from image_gen_mcp.core.config import get_api_key

# 获取API密钥
api_key = get_api_key(None, service="imgbb")
...
```python
from image_gen_mcp.apps.cell_cover.utils.image_uploader import upload_to_github
from image_gen_mcp.core.config import get_api_key

# 获取API密钥
github_token = get_api_key(None, service="github")
...
如果需要在创建封面时使用上传的图片作为参考图像，可以这样使用：

```bash
# 先上传图片到ImgBB
./scripts/upload_image.sh upload ./reference_image.png
# 或上传到GitHub
./scripts/upload_image.sh upload -s github ./reference_image.png
# 复制返回的URL

# 然后在创建封面时使用--cref参数
crc create -c ca -var scientific -v 6 --cref "复制的URL"
```
