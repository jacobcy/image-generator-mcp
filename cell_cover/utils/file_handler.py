...
def save_image_metadata(logger, image_id, job_id, filename, filepath, url, prompt, concept, variations=None, components=None, seed=None, original_job_id=None):
...
def save_action_metadata(logger, image_id, new_job_id, original_job_id, action_code, filename, filepath, url, prompt, concept, seed=None):
    """将后续操作 (Upscale/Variation) 的结果元数据保存到 actions_metadata.json。
...
def download_and_save_image(logger, image_url, job_id, prompt, concept_key,
                          ...
                          original_job_id=None, action_type=None, action_index=None,
    ...
    Args:
        ...
        action_type (str, optional): 'U' for Upscale, 'V' for Variation, 'R' for Reroll.
        action_index (int, optional): The index (1-4) for Upscale/Variation.
    ...
    action_log_str = f"{action_type}{action_index}" if action_type and action_index else 'N/A'
    ...
    filename = ""
    action_code = None # Define action_code variable
    try:
        # Get file extension (common logic)
        parsed_url = requests.utils.urlparse(image_url)
        path_part = parsed_url.path
        file_ext = os.path.splitext(path_part)[-1].lower() or ".png"
        if not file_ext.startswith('.') or len(file_ext) > 5: file_ext = ".png"

        concept_part = concept_key if concept_key else "no_concept"
        s_concept = sanitize_filename(concept_part)

        if action_type and action_index is not None and original_job_id:
            # Subsequent action filename: concept-origJobPrefix-ActionIndex-timestamp.ext
            original_job_id_prefix = original_job_id[:6]
            action_code = f"{action_type.upper()}{action_index}" # Capture action code
            filename = f"{s_concept}-{original_job_id_prefix}-{action_code}-{timestamp}{file_ext}"
            logger.info(f"生成操作结果文件名: {filename}")
        else:
            # Initial creation filename: concept-jobPrefix-variation-style-timestamp.ext
            job_id_prefix = job_id[:6] if job_id else "nojobid"
            variation_part = '-'.join(variation_keys) if variation_keys else "no_variation"
            style_part = '-'.join(global_style_keys) if global_style_keys else "no_style"
            s_variation = sanitize_filename(variation_part)
            s_style = sanitize_filename(style_part)
            filename = f"{s_concept}-{job_id_prefix}-{s_variation}-{s_style}-{timestamp}{file_ext}"
            logger.info(f"生成初次创建文件名: {filename}")

    except Exception as fn_e:
        logger.error(f"生成文件名时出错: {fn_e}", exc_info=True)
        # Fallback filename
        fallback_id = job_id or original_job_id or str(uuid.uuid4())[:8]
        filename = f"error_fallback_{fallback_id}_{timestamp}.png"
        logger.warning(f"使用备用文件名: {filename}")

    if not filename: # Should not happen with fallback, but safety check
         logger.error("无法生成有效的文件名，下载中止。")
         return None
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Call save_image_metadata, passing original_job_id if it exists
            metadata_saved = False

            if original_job_id and action_code:
                # Call save_action_metadata
                logger.info("调用 save_action_metadata 保存操作结果...")
                metadata_saved = save_action_metadata(
                    logger=logger,
                    image_id=image_id,
                    new_job_id=job_id, # Pass the current job_id as new_job_id
                    original_job_id=original_job_id,
                    action_code=action_code,
                    filename=filename,
                    filepath=filepath,
                    ...
                    concept=concept_key, # Pass concept
                    seed=seed # Pass seed if available
                )
            else:
                # Call save_image_metadata for initial creation
                logger.info("调用 save_image_metadata 保存初始图像...")
                metadata_saved = save_image_metadata(
                    logger=logger,
                    image_id=image_id,
                    job_id=job_id, # Pass current job_id
                    filename=filename,
                    filepath=filepath,
                    url=image_url,
                    prompt=prompt,
                    concept=concept_key,
                    variations=variation_keys,
                    seed=seed
                )

            if not metadata_saved:
                logger.warning(f"图像已下载，但保存元数据失败 (调用了 {'save_action_metadata' if original_job_id else 'save_image_metadata'})。")
